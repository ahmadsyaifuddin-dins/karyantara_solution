...
class SyncProjectsToGoogleSheet extends Command
{
    ...
    public function handle(GoogleSheetService $googleSheetService)
    {
        ...
        // AMBIL SEMUA DATA (KARENA SEMUANYA SEKARANG PUBLIK)
        $projects = Project::all();

        if ($projects->isEmpty()) {
            $this->warn('Tidak ada data proyek di database untuk disinkronisasi.');
            return;
        }
        ...
        $this->info('Berhasil! '.count($projects).' data proyek telah disinkronkan.');
    }
}