<div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden flex flex-col max-h-[600px]">
    <div class="px-6 py-5 border-b border-amber-100 bg-amber-50/30 flex items-center justify-between">
        <h3 class="font-bold text-[#1E293B]"><i class="fa-solid fa-file-word text-amber-500 mr-2"></i> Histori Penyusun
            Naskah</h3>
        <span class="bg-amber-100 text-amber-700 text-xs px-2 py-1 rounded-md font-bold">{{ $writerProjects->count() }}
            Proyek</span>
    </div>

    <div class="divide-y divide-gray-100 overflow-y-auto flex-1">
        @forelse($writerProjects as $project)
            <div class="p-5 hover:bg-gray-50 transition-colors {{ !$project->is_writer_paid ? 'bg-red-50/10' : '' }}">
