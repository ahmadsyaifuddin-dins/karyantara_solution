<div class="overflow-x-auto">
    <table class="min-w-full divide-y divide-gray-200">
        <thead class="bg-gray-50/80">
            <tr>
                <th class="px-6 py-4 text-left text-xs font-bold text-[#1E293B] uppercase tracking-wider">Jabatan &
                    Preview Badge</th>
                <th class="px-6 py-4 text-left text-xs font-bold text-[#1E293B] uppercase tracking-wider">Divisi
                    (Department)</th>
                <th class="px-6 py-4 text-left text-xs font-bold text-[#1E293B] uppercase tracking-wider">Atasan Langsung
                </th>
                <th class="px-6 py-4 text-left text-xs font-bold text-[#1E293B] uppercase tracking-wider">Total Karyawan
                </th>
                <th class="px-6 py-4 text-right text-xs font-bold text-[#1E293B] uppercase tracking-wider">Aksi</th>
            </tr>
        </thead>
            ...
            @forelse ($positions as $item)
                <tr class="hover:bg-slate-50/50 transition-colors duration-200">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap">
                        @if ($item->parent)
                            <div class="flex items-center gap-2 text-sm text-slate-700">
                                <i class="fa-solid fa-arrow-turn-up fa-rotate-90 text-amber-500 text-xs"></i>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                        {{-- Menggunakan users_count hasil dari withCount() di Controller --}}
                        <div class="flex items-center gap-2">
                            <i class="fa-solid fa-users text-gray-400"></i>
                            <span class="{{ $item->users_count > 0 ? 'font-bold text-[#1E293B]' : 'text-gray-400' }}">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <div class="flex justify-end items-center gap-4">
                            ...
                            @if ($item->users_count > 0 || $item->children()->count() > 0)
                                <button type="button" class="text-gray-300 cursor-not-allowed flex items-center gap-1"
                                    title="Tidak dapat dihapus karena sedang digunakan atau memiliki bawahan">
                                    <i class="fa-solid fa-trash"></i> Hapus
                                </button>
                            @else
                                <form action="{{ route('admin.positions.destroy', $item->id) }}" method="POST"
                ...
                <tr>
                    <td colspan="5" class="px-6 py-12 text-center text-gray-500">
                        <div class="flex flex-col items-center justify-center">
                            ...
                            <p class="text-sm">Silakan tambah jabatan baru untuk menstrukturisasi perusahaan Anda.</p>
                        </div>
                    </td>
                </tr>
            @endforelse
